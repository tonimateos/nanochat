...
# On MPS, the default watermark ratio of 0.7 can be too conservative for training large models.
# Setting it to 0.0 allows the OS to manage the memory more flexibly, which can prevent OOM.
...
def main():
    ...
    parser.add_argument("--compile", type=int, default=1, help="use torch.compile (0=no, 1=yes)")
    # Model loading
    ...
    if args.compile:
        ...
        print0("Model compilation enabled.")
    else:
        print0("Model compilation disabled (running in eager mode).")
    ...
    def sft_data_generator_bos_bestfit(split, buffer_size=100):
        ...
        def refill_buffer():
            ...
            if len(conv_buffer) < buffer_size:
                print0(f" DEBUG: Refilling data buffer (cursor: {cursor})...")
            while len(conv_buffer) < buffer_size:
                # print0(f" DEBUG: Fetching conversation {cursor}...")
                conversation = dataset[cursor]
                # print0(f" DEBUG: Rendering conversation {cursor}...")
        ...
        while True:
            ...
            if split == "train":
                ...
                if args.num_iterations > 0:
                    state['approx_progress'] = it / args.num_iterations
                else:
    ...
    while True:
        ...
        if ddp:
            last_step_tensor = torch.tensor(state['last_step'], dtype=torch.int32, device=device)
            dist.all_reduce(last_step_tensor, op=dist.ReduceOp.MAX)
            state['last_step'] = bool(last_step_tensor.item())

        # once in a while: evaluate the val bpb (all ranks participate)
        if state['last_step'] or (args.eval_every > 0 and step % args.eval_every == 0):
            model.eval()
        ...
        chatcore_results = {}
        if args.chatcore_every > 0 and (state['last_step'] or (step > 0 and step % args.chatcore_every == 0)):
            model.eval()
            eval_engine = Engine(orig_model, tokenizer)
            ...
            for task_name in all_tasks:
                ...
                acc = run_chat_eval(task_name, orig_model, tokenizer, eval_engine,
        ...
        for micro_step in range(grad_accum_steps):
            ...
            progress = max(progress, state['approx_progress'])
        # step the optimizer
        ...
        print0(f"step {step:05d} ({pct_done:.2f}%) | loss: {debiased_smooth_loss:.6f} | lrm: {lrm:.2f} | dt: {dt * 1000:.2f}ms | tok/sec: {tok_per_sec:,} | mfu: {mfu:.2f} | epoch: {state['current_epoch']} | total time: {total_training_time/60:.2f}m")
        if step % 10 == 0:
            wandb_run.log({
                "step": step,
                "total_training_flops": flops_so_far,
